...
const agregarCurso = (req, res) => {
    ...
    database_1.default.run("INSERT INTO Cursos(id_curso, nombre_curso, descripcion, duracion_horas) VALUES (?, ?, ?, ?)", [id_curso, nombre_curso, descripcion, duracion_horas], (err) => {
        if (err) {
...
const verCursos = (req, res) => {
    database_1.default.all("SELECT * FROM Cursos", (err, rows) => {
        if (err) {
            res.status(500).json({ status: "error", message: err.message });
        }
        else {
...
const verCursoPorId = (req, res) => {
    ...
    database_1.default.get("SELECT * FROM Cursos WHERE id_curso = ?", [id_curso], (err, row) => {
        if (err) {
            res.status(500).json({ status: "error", message: err.message });
        }
        else if (row) {
...
const actualizarCurso = (req, res) => {
    ...
    database_1.default.get("SELECT * FROM Cursos WHERE id_curso = ?", [id_curso], (err, row) => {
        ...
        database_1.default.run("UPDATE Cursos SET nombre_curso = ?, descripcion = ?, duracion_horas = ? WHERE id_curso = ?", [updatedNombre, updatedDescripcion, updatedHoras, id_curso], (updateErr) => {
            if (updateErr) {
...
const borrarCurso = (req, res) => {
    const id_curso = req.params.id_curso;
    database_1.default.run("DELETE FROM Cursos WHERE id_curso = ?", [id_curso], function (err) {
        if (err) {
